...
function dispatchMinistryInterest({ name, email, phone, message, ministry }) {
  const oneLine = (message || "").replace(/\s+/g, " ").slice(0, 200);
  console.log(`[MINISTÉRIO] ${ministry} | ${name} | ${email} | ${phone} | ${oneLine}`);
  ...
  //   from: process.env.MAIL_FROM,
  //   to: process.env.MAIL_TO,
  //   subject: `Interesse no ministério ${ministry} — ${name}`,
  //   text: `Ministério: ${ministry}\nNome: ${name}\nE-mail: ${email}\nTelefone: ${phone}\n\n${message}`,
  // });
  // ------------------------------------------------------------------
...
module.exports = { dispatchContactMessage, dispatchMinistryInterest };